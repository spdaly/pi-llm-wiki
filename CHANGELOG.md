...
### Fixed
- **OpenKnowledge metadata exclusion**: discovery no longer treats root or nested `.ok/templates/*.md` files as knowledge pages. Registry counts, backlinks, and generated directory navigation exclude those templates, and obsolete-index pruning leaves `.ok/` metadata untouched.
- **Guardrails only inspected `write` and `edit`**: interception was a two-name type-guard whitelist, so every other path-shaped mutation tool wrote into `raw/**`, `meta/events.jsonl` and generated OKF projections unblocked — and never set the dirty flag, so those mutations also skipped the `turn_end` metadata rebuild. Matching is now keyed on the tool name across `write`, `edit`, `ast_edit`, `apply_patch`, `multi_edit` and `notebook_edit`, and omp's device-write indirection is unwrapped: it invokes device and extension tools as `write` to an `xd://<device>` pseudo-path with the real arguments JSON-encoded in `content` (verified: `write {path: "xd://wiki_ingest", content: "{}"}`), so an `ast_edit` codemod is scanned by its real target list, an unparseable payload is blocked, and `xd://wiki_*` calls pass through because those tools legitimately own `raw/**` and `meta/**`. `bash` and `eval` remain unguardable by path scanning on either host, and glob-scoped codemods are only checked against the literal paths they name — both are now stated in `docs/architecture.md`.
